...
import { useAudio } from '../context/AudioContext'; // Assuming this path is correct
import { Dice1 as DiceD20Icon, Info, Plus, X } from 'lucide-react';
import DiceBox, { DiceRoll } from '@3d-dice/dice-box'; // Import DiceBox and its types

interface CustomDiceResult { // Our internal structure
  type: number; 
  value: number; 
  id: string;
}
...
export const DiceRoller: React.FC = () => {
  ...
  const diceContainerRef = useRef<HTMLDivElement>(null); // Ref for the dedicated DiceBox div
  const diceBoxInstanceRef = useRef<DiceBox | null>(null);
  const [isDiceBoxReady, setIsDiceBoxReady] = useState(false);

  // useEffect for DiceBox Initialization
  useEffect(() => {
    let currentBoxInstance: DiceBox | null = null; 

    if (document.getElementById("dice-box-container") && !diceBoxInstanceRef.current) {
      console.log("Initializing DiceBox with LOCAL assets (default theme)...");
      
      currentBoxInstance = new DiceBox({
        container: "#dice-box-container", // ID of the dedicated div
        assetPath: '/assets/dice-box/', 
        theme: 'default',
        offscreen: true, 
        scale: 30, // Your current scale
      });

      diceBoxInstanceRef.current = currentBoxInstance;

      currentBoxInstance.init()
        .then(() => {
          if (diceBoxInstanceRef.current === currentBoxInstance) {
            console.log("DiceBox Initialized with LOCAL assets!");
            setIsDiceBoxReady(true);
          } else {
            console.log("DiceBox initialization was superseded. This instance will be cleaned up by its own effect return.");
            currentBoxInstance?.clear?.(); 
          }
        })
        .catch(err => {
          console.error("DiceBox init with LOCAL assets failed. Error object:", err);
          if (err && err.message) {
    ...
    // Cleanup function
    return () => {
      console.log("useEffect cleanup for DiceBox: Attempting cleanup.");
      
      const instanceToClean = currentBoxInstance || diceBoxInstanceRef.current;

      if (instanceToClean) {
        console.log("Cleaning DiceBox instance.");
        if (typeof instanceToClean.clear === 'function') {
          try {
            instanceToClean.clear(); 
            console.log("Called instanceToClean.clear()");
          } catch (e) {
            console.warn("instanceToClean.clear() threw an error during cleanup:", e);
          }
        }
        // If the library had a specific .destroy() or .dispose() method, it would be called here.
        // e.g., instanceToClean.destroy?.();
      }

      // Only nullify the main ref if it's pointing to the instance this effect managed
      // or if it matches the instance we captured for cleanup.
      if (diceBoxInstanceRef.current === instanceToClean) {
          diceBoxInstanceRef.current = null;
      }
      setIsDiceBoxReady(false); 
      // React will handle unmounting the #dice-box-container div and its canvas child
      // when the DiceRoller component unmounts.
    };
  }, []); // Empty dependency array for one-time setup for local assets

  // useEffect for loading saved rolls from localStorage
  useEffect(() => {
    ...
    if (savedRollsData) {
      try {
        ...
        console.error("Error parsing saved rolls from localStorage", e);
        setSavedRolls([]);
      }
    } else {
      const defaultRolls = [
  ...
  // useEffect for saving rolls to localStorage when they change
  useEffect(() => {
  ...
  const rollDice = async (customRoll?: SavedRoll) => {
    if (!diceBoxInstanceRef.current || !isDiceBoxReady) {
      console.warn("DiceBox not ready or not initialized.");
      return;
    }
    setIsRolling(true);
    setTotal(null); 
    ...
    try {
      if (currentAdvantage !== 'normal' && typeOfDice === 20) {
        ...
        if (!diceBoxRolledResults || diceBoxRolledResults.length !== 2) {
          throw new Error("Advantage/disadvantage roll did not return 2 dice.");
        }
        diceBoxRolledResults.sort((a, b) =>
        ...
        setDiceResults(diceBoxRolledResults.map((d, index) => ({
            type: typeOfDice,
            value: d.value,
            id: `${Date.now()}-adv-${index}`
        })));
      } else {
      ...
      console.error("Error rolling dice with DiceBox:", error);
      setIsRolling(false);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      >
        ...
        {/* 3D Dice Display Container - Outer wrapper for positioning and messages */}
        <div className="relative min-h-[300px] md:min-h-[400px] bg-transparent rounded-lg mb-6 overflow-hidden border border-dashed border-foreground/30">
          
          {/* Conditional Messages - Managed by React, sibling to DiceBox's target */}
          ...
          {/* Dedicated, empty div for DiceBox to attach its canvas to */}
          <div 
              ...
              <div key={index} className="bg-muted/30 rounded-lg p-3 group relative">
                ...
                <div onClick={() => handleUseSavedRoll(saved)} className="cursor-pointer">
                  ...
                  <p className="text-sm text-foreground/70">
                    {saved.dice}d{saved.diceType}{saved.modifier !== 0 ? (saved.modifier > 0 ? `+${saved.modifier}` : saved.modifier) : ''}
                  </p>
                  <button
                    tabIndex={-1} 
          ...
          >
            ...
            >
              ...
              <p className="mb-4 text-foreground/70">
                Current: {numberOfDice}d{diceType}{modifier !== 0 ? (modifier > 0 ? `+${modifier}` : modifier) : ''}
              </p>
              <div className="mb-4">
      ...
      {/* Common Dice Rolls Info section */}
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
